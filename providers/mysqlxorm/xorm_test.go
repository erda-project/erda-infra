...
func Test_config_url(t *testing.T) {
	type fields struct {
		...
		MySQLShowSQL        bool
		MySQLProperties     string
		MySQLTLS            string
		MySQLPingWhenInit   bool
		MySQLPingTimeoutSec uint64
	}
	tests := []struct {
		name   string
		fields fields
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &config{
				...
				MySQLShowSQL:        tt.fields.MySQLShowSQL,
				MySQLProperties:     tt.fields.MySQLProperties,
				MySQLTLS:            tt.fields.MySQLTLS,
				MySQLPingWhenInit:   tt.fields.MySQLPingWhenInit,
				MySQLPingTimeoutSec: tt.fields.MySQLPingTimeoutSec,
			}
